# 列出指定聊天室在 Qdrant 中的主角（主人公）背景切片
# 用法: list-protagonist conv_xxxxxxxxx
param(
...
$body = @{
    filter = @{
        must = @(
            ...
            @{ key = "type"; match = @{ value = "protagonist_background" } }
        )
    }
    limit = 200
    with_payload = $true
...
Write-Host "資料種類: 主角（主人公）背景（collection=characters, type=protagonist_background）"
